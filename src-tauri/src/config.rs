...
pub fn normalize_whoop_address(address: &str) -> Result<String, String> {
    ...
    // iOS uses CoreBluetooth UUID addresses which btleplug stores as lowercase.
    // macOS/Linux use MAC addresses where case doesn't matter for lookup,
    // so lowercase works on all platforms.
    Ok(trimmed_address.to_ascii_lowercase())
}
